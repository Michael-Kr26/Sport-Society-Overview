...
const API_ACCESS = Object.freeze([
    ...
    { prefix: '/api/roster-operations/parity', minimumRole: 'admin' },
    { prefix: '/api/roster-export/sharepoint', minimumRole: 'admin' },
    { prefix: '/api/roster-export/history', minimumRole: 'admin' },
    { prefix: '/api/roster-export', minimumRole: 'manager' },
    { prefix: '/api/roster-operations', minimumRole: 'manager' },
...
async function ensureColumn(db, tableName, columnName, definition) {
    ...
    try {
        ...
        // Startupmodules kunnen dezelfde idempotente migratie tegelijk bereiken.
        // Wanneer een andere verbinding de kolom net heeft toegevoegd, is de
        // uiteindelijke schema-state correct en behandelen we dat als succes.
        if (error?.code === 'SQLITE_ERROR' && /duplicate column name/i.test(String(error.message || ''))) {
            const refreshed = await all(db, `PRAGMA table_info(${tableName})`);
            if (refreshed.some((column) => column.name === columnName)) return;
        }
        throw error;
    }
}
...
async function ensureRosterAccessSchema(db) {
    ...
    await exec(db, `
        ...
        BEGIN
            ...
            SELECT NEW.id, l.id, 0, 0, date('now', 'localtime')
            FROM locations l
            WHERE l.name=NEW.location COLLATE NOCASE
              AND NOT EXISTS (
                  SELECT 1 FROM user_location_scopes s
                  WHERE s.user_id=NEW.id AND s.location_id=l.id
    ...
    // R7 beleidswijziging: Managers mogen geen rooster plannen of publiceren.
    // Bestaande locatie-scopes blijven bruikbaar als organisatorische toewijzing,
    // maar de roosterrechten worden bij iedere startup/migratie geneutraliseerd.
    await run(db, `UPDATE user_location_scopes
        SET can_edit_roster=0, can_publish_roster=0
        WHERE user_id IN (SELECT id FROM users WHERE role='manager')
          AND (can_edit_roster <> 0 OR can_publish_roster <> 0)`);
    ...
    for (const manager of managers) {
        ...
        await run(db, `INSERT INTO user_location_scopes
            (user_id, location_id, can_edit_roster, can_publish_roster, effective_from)
            VALUES (?, ?, 0, 0, ?)`, [manager.id, location.id, effectiveFrom]);
    }
}
...
async function migrateRosterAccess(db) {
    ...
    return {
        managerScopes: Number((await get(db, `SELECT COUNT(*) AS count
            FROM user_location_scopes s
            INNER JOIN users u ON u.id=s.user_id
            WHERE u.role='manager'`))?.count || 0)
    };
}
