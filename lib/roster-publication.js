...
async function createRosterPublicationWorkflow(db) {
    ...
    async function cmlProjection(publicationId, actorUserId, reason) {
        ...
        for (const period of periods) {
            ...
            for (const employeeId of employeeIds) {
                const name = await employeeName(employeeId, new Map());
    ...
    async function projectSideEffects(publicationId, actorUserId, reason) {
        try {
            await exec(db, 'BEGIN IMMEDIATE TRANSACTION;');
            const cml = await cmlProjection(publicationId, actorUserId, reason);
    ...
    async function publish({ actorUserId, versionIds, reason = null, referenceWeekStart }) {
        await domain.AuthorizationService.assertCanPublish(actorUserId);
        ...
        const sideEffects = await projectSideEffects(result.publicationId, actorUserId, reason);
        return {
            ...result,
            sideEffects,
