...
async function createRosterPlanner(db) {
    ...
    async function assertAdminPlannerUser(userId) {
        ...
        if (user.role !== 'admin') {
            throw plannerError('Alleen Admin mag het rooster plannen.', 'ROSTER_EDIT_FORBIDDEN', 403);
        }
        return user;
    }

    async function listLocations(userId, weekStart, canPlan) {
        ...
        for (const location of locations) {
            output.push({
                ...location,
                canEdit: Boolean(canPlan && await domain.AuthorizationService.canEditLocation(userId, location.id, weekStart))
            });
        }
        return output;
    }
    ...
    async function buildContext({ userId, locationCode, weekStart, view = 'auto' }) {
        ...
        const canEdit = Boolean(canPlan && await domain.AuthorizationService.canEditLocation(userId, location.id, weekStart));
        const canPublish = Boolean(canPlan && await domain.AuthorizationService.canPublish(userId));
        ...
        const locations = await listLocations(userId, weekStart, canPlan);
        ...
        return {
            ...
            views: {
                ...
                draftVersionId: canEdit ? state.draft?.id || null : null,
                draftRevision: canEdit ? state.draft?.revision ?? null : null,
    ...
    async function ensureDraft({ userId, locationCode, weekStart, changeNote = 'Weekplanner' }) {
        await assertAdminPlannerUser(userId);
        assertWeekStart(weekStart);
        const location = await resolveLocation(locationCode);
    ...
    async function versionForMutation(userId, versionId) {
        ...
        if (!await domain.AuthorizationService.canEditLocation(userId, version.locationId, version.weekStart)) {
            throw plannerError('Alleen Admin mag het rooster wijzigen.', 'ROSTER_EDIT_FORBIDDEN', 403);
        }
        return version;
    }
