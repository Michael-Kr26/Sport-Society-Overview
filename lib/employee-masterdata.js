...
async function listActiveLocations(db) {
    return all(db, `SELECT code, name, timezone, sort_order AS sortOrder
        FROM locations
        WHERE is_active=1
...
async function createEmployee(db, input = {}) {
    ...
    const primaryLocationCode = cleanName(input.primaryLocationCode).toUpperCase();
    ...
    if (!primaryLocationCode) {
        throw requestError(400, 'PRIMARY_LOCATION_REQUIRED', 'Kies een primaire locatie voor de medewerker.');
    }
    const primaryLocation = await get(db, `SELECT id, code, name
        FROM locations WHERE code=? COLLATE NOCASE AND is_active=1`, [primaryLocationCode]);
    if (!primaryLocation) {
        throw requestError(400, 'INVALID_PRIMARY_LOCATION', 'De gekozen primaire locatie bestaat niet of is niet actief.');
    }
    ...
    try {
        ...
        await run(db, `INSERT INTO employee_location_eligibility
            ...
            primaryLocation.id,
            startsOn || knownFrom,
            'Primaire locatie bij aanmaak via Medewerkers'
        ]);
        await run(db, 'COMMIT');
        return {
            created: true,
            employee: {
                ...
                archivedAt: null,
                primaryLocationCode: primaryLocation.code,
                eligibleLocationCodes: [primaryLocation.code],
                locations: [primaryLocation.name]
            }
        };
    } catch (error) {
