(() => {
    ...
    function employeeCard(employee) {
        ...
        const isFuture = employee.isActive && employee.activeFrom && employee.activeFrom > today();
        const statusLabel = !employee.isActive ? 'Verwijderd' : isFuture ? 'Toekomstig' : 'Actief';
        const statusClass = !employee.isActive ? ' is-inactive' : isFuture ? ' is-future' : '';

        return `
            <article class="employee-item${employee.isActive ? '' : ' is-inactive'}" data-employee-card="${escapeHtml(employee.employeeName)}" tabindex="-1">
                <div class="employee-item-header">
                    <div>
                        <div class="employee-item-title">
                            ...
                            <span class="employee-badge${statusClass}">${statusLabel}</span>
                        </div>
    ...
    function revealExistingEmployee(employee) {
        searchInput.value = employee.employeeName;
        statusFilter.value = 'all';
        contractFilter.value = 'all';
        renderEmployees();

        window.requestAnimationFrame(() => {
            const card = [...employeeList.querySelectorAll('[data-employee-card]')]
                .find((item) => employeeKey(item.dataset.employeeCard) === employeeKey(employee.employeeName));
            if (!card) return;
        ...
        const statusText = employee.isActive ? 'bestaande medewerker' : 'verwijderde medewerker';
        setMessage(`${employee.employeeName} bestaat al en is hieronder geopend als ${statusText}. Pas daar de instellingen aan${employee.isActive ? '.' : ' of klik op Herstellen.'}`, 'success');
    }
    ...
    async function addEmployee(event) {
        ...
        const isContract = newType.value === 'contract';
        const existing = employees.find((employee) => employeeKey(employee.employeeName) === employeeKey(name));
        if (existing) {
            revealExistingEmployee(existing);
            return;
        }
